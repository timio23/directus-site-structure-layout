...
const { itemKey, itemSort, itemDepth, itemParent, snapStep, disabled } = props;
...
function useSortable({
	...
	function getCurrentDepth() {
		if (draggedItemIndex.value === null || !nestable.value)
			return 0;
    if (items.value[draggedItemIndex.value] === undefined) return 0;
	...
	function onSortEnd() {
		...
		function updateDepth() {
			items.value[draggedItemIndex.value!]![itemDepth] = intendedDepthChange.value;
		}
		...
		function updateParent() {
			...
			if (
				...
				items.value[draggedItemIndex.value!]![itemParent!] = parentId;
